(function (root, factory) {
  ...
  function artifactHasAncestorType(artifact, artifactById, requiredType) {
    const pending = Array.isArray(artifact.parent_artifact_ids) ? artifact.parent_artifact_ids.slice() : [];
    ...
    while (pending.length) {
      ...
      for (const grandparentId of parent.parent_artifact_ids || []) {
  ...
  function modelPromptHasUnapprovedRawPath(artifact, artifactById) {
    const pending = (artifact.parent_artifact_ids || []).map((artifactId) => ({ artifactId, approvedSeen: false }));
    const visited = new Set();
