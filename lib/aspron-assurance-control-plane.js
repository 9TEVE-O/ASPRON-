(function (root, factory) {
  ...
  function normalizeParentArtifactIds(value) {
  ...
  function createArtifactRecord(options) {
    ...
    const parentArtifactIdsForUse = normalizeParentArtifactIds(parentArtifactIds);
    ...
    return {
      ...
      hash: toText(options.hash || createDemoHash([artifactType, parentArtifactIdsForUse.join("|"), classification, approvalState].join("::"))),
  ...
  function artifactHasAncestorType(artifact, artifactById, requiredType) {
    const pending = normalizeParentArtifactIds(artifact.parent_artifact_ids).slice();
    ...
    while (pending.length) {
      ...
      if (!parent) continue;
      if (parent.artifact_type === requiredType) return true;

      for (const grandparentId of normalizeParentArtifactIds(parent.parent_artifact_ids)) {
  ...
  function modelPromptHasUnapprovedRawPath(artifact, artifactById) {
    const pending = normalizeParentArtifactIds(artifact.parent_artifact_ids).map((artifactId) => ({ artifactId, approvedSeen: false }));
    const visited = new Set();

    while (pending.length) {
      const current = pending.pop();
      const visitKey = `${current.artifactId}:${current.approvedSeen}`;
      if (visited.has(visitKey)) continue;
      visited.add(visitKey);

      const parent = artifactById.get(current.artifactId);
      if (!parent) continue;
      const approvedSeen = current.approvedSeen || parent.artifact_type === "approved_ai_input";
      if (parent.artifact_type === "raw_original" && !approvedSeen) return true;

      for (const grandparentId of normalizeParentArtifactIds(parent.parent_artifact_ids)) {
        pending.push({ artifactId: grandparentId, approvedSeen });
      }
    }
  ...
  function validateLineage(record, errors) {
    ...
    record.lineage_artifacts.forEach((artifact, index) => {
      ...
      if (!artifactById.has(artifact.artifact_id)) artifactById.set(artifact.artifact_id, artifact);

      if (artifact.parent_artifact_ids !== undefined && !Array.isArray(artifact.parent_artifact_ids)) {
        errors.push(`lineage_artifacts[${index}] parent_artifact_ids must be an array`);
      }
    ...
    record.lineage_artifacts.forEach((artifact, index) => {
      for (const parentId of normalizeParentArtifactIds(artifact.parent_artifact_ids)) {
    ...
    const rawOriginals = record.lineage_artifacts.filter((artifact) => artifact.artifact_type === "raw_original");
    const approvedInputs = record.lineage_artifacts.filter((artifact) => artifact.artifact_type === "approved_ai_input" && artifact.approval_state === "approved");
    const modelPrompts = record.lineage_artifacts.filter((artifact) => artifact.artifact_type === "model_prompt");

    if (rawOriginals.length === 0) {
    ...
    approvedInputs.forEach((artifact) => {
      if (!artifactHasAncestorType(artifact, artifactById, "raw_original")) {
        errors.push(`approved_ai_input must descend from raw_original: ${artifact.artifact_id}`);
      }
    });

    modelPrompts.forEach((artifact) => {
      if (
        !artifactHasAncestorType(artifact, artifactById, "approved_ai_input") ||
        modelPromptHasUnapprovedRawPath(artifact, artifactById)
      ) {
        errors.push(`model_prompt raw ancestry must pass through approved_ai_input: ${artifact.artifact_id}`);
      }
    });
  }
