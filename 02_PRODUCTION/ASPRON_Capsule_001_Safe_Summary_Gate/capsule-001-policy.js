const RiskRules = typeof require === "function" ? require("../../lib/aspron-risk-rules.js") : window.ASPRONRiskRules;
...
function fp(value) { return RiskRules.createPayloadFingerprint(text(value)); }
...
function classifyRisk(c) {
  notDissolved(c);
  stateIs(c, STATES.INPUT_RECEIVED, "INVALID_STATE_FOR_CLASSIFICATION");
...
function createRedactionCandidate(c) {
  notDissolved(c);
  stateIs(c, [STATES.RISK_CLASSIFIED, STATES.RAW_ACCESS_BLOCKED], "INVALID_STATE_FOR_REDACTION");
...
function approveCandidate(c, reviewer = "demo_reviewer") {
  notDissolved(c);
  stateIs(c, STATES.REDACTION_CANDIDATE, "INVALID_STATE_FOR_APPROVAL");
...
function createAiVisibleInput(c) {
  notDissolved(c);
  stateIs(c, STATES.APPROVED, "INVALID_STATE_FOR_AI_VISIBLE_INPUT");
...
function createSafeSummary(c) {
  notDissolved(c);
  stateIs(c, STATES.AI_VISIBLE_INPUT_READY, "INVALID_STATE_FOR_SAFE_SUMMARY");
  c.safe_summary = {
...
function createEvidenceReceipt(c) {
  notDissolved(c);
  stateIs(c, STATES.SAFE_SUMMARY_READY, "INVALID_STATE_FOR_RECEIPT");
...
function dissolveCapsule(c) {
  notDissolved(c);
  stateIs(c, STATES.EVIDENCE_RECEIPT_READY, "INVALID_STATE_FOR_DISSOLVE");
  ...
  c.risks = [];
  c.dissolved = true;
  c.state = STATES.DISSOLVED;
  return c;
}

module.exports = { STATES, PolicyGateError, createCapsule, replaceRawInput, classifyRisk, attemptRawAgentAccess, createRedactionCandidate, approveCandidate, createAiVisibleInput, createSafeSummary, createEvidenceReceipt, dissolveCapsule };
